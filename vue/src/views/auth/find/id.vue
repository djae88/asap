    ...
    <v-sheet class="pt-16 pb-16 rounded-10" color="#f9f9f9">
        <v-sheet class="px-8 py-16 mx-auto rounded-lg" width="480" outlined>
            ...
            <v-card-text class="d-flex justify-center">
                ...
                <v-btn to="/auth/find/password" small text color="grey darken-1">비밀번호 찾기</v-btn>
            </v-card-text>
        </v-sheet>
    </v-sheet>
</template>
...
export default {
    ...
    methods: {
        submit(){
            // 이메일로 회원 찾기
            this.$http.post('/api/auth/find_id/search', {
                params: {
                    email: this.email
                }
            }).then((res) => {
                if(!res.data.length){
                    ...
                    // 이메일 발송
                    this.$http.post('/api/auth/find_id/send', {
                        params: {
                            email: this.email,
                        ...
                        if(res.data){
                            alert("아이디 찾기 안내 이메일이 정상적으로 발송되었습니다.")
                        }
                    })
                }
            })
